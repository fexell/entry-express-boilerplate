...
AuthMiddleware.Authenticate                 = async (req, res, next) => {
  try {
    ...
    if(accessToken) {
      ...
      const decodedAccessToken              = await JwtHelper.ValidateAndDecodeToken(accessToken, 'access')
      ...
      const decodedRefreshToken             = await JwtHelper.ValidateAndDecodeToken(refreshTokenRecord.token, 'refresh')
